<?php
/*
 * Plugin Name: RSS Single Prog.
 * Plugin URI: 
 * Description: Visualizza gli RSS relativi al programma
 * Version: 2.0
 * Author: Giampaolo
 * Author URI: 
*/
...
class RssProgram extends WP_Widget {


	function RssProgram() {
		parent::WP_Widget(
			'simplerssfeedswidget',
			__( 'RSS Single Prog.', 'simplerssfeedswidget' ),
			array(	'classname' => 'simplerssfeedswidget', 'description' => __( 'Visualizza gli RSS relativi al programma', 'simplerssfeedswidget' ) )
		);
	}
	...
	function form( $instance )
	{
		...
		if( empty( $title ) )
			$title = __( 'Notizie dal Blog', 'simplerssfeedswidget' );
	...
	function widget( $args, $instance )
	{
		...
		$post_meta = get_post_meta(get_the_ID($post->ID), 'metakey_feed', true);
	        $meta_values = json_decode($post_meta,true);
		$rssurl = $meta_values['feed'];
		if( $rssurl != '') {
		    $rss = $rssurl;
		} else {
			$rss = $instance[ 'rss' ];
		...
		{
			// Yes, we do
			usort( $displayedItems, 'RssProgram::sort' );
  ...
  add_action( 'widgets_init', create_function('', 'return register_widget("RssProgram");') ); ?>